...
<?php
  ...
  //NEED TO DELETE MEDIUM AND THUMBS? OR USE UPDATE INSTEAD?
  ...
  $stmt = oci_parse($conn, 'insert into pacs_images (record_id, image_id, thumbnail, regular_size, full_size) '
        .'values(:recordID, :imageID, empty_blob(), empty_blob(), empty_blob()) returning thumbnail, regular_size, full_size into :blobthumb, :blobregular, :blobdata');
  oci_bind_by_name($stmt, ':recordID', $recordID);
  oci_bind_by_name($stmt, ':blobthumb', $lob, -1, OCI_B_BLOB);
  oci_bind_by_name($stmt, ':blobregular', $lob, -1, OCI_B_BLOB);
